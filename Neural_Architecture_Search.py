...
class MyModelSpace(ModelSpace):
    def __init__(self):
        ...
        # 使用 nni.choice 动态决定层数量
        layers_options = [
            nn.Sequential(
                nn.Linear(28 * 28, 256),  # 第一层，将28x28的图片展平后输入，输出128个节点
                nn.ReLU(),
            ),
            nn.Sequential(
            ...
            nn.Sequential(
                ...
                nn.Linear(128, 256),  # 第二层，接收第一层的128个节点，输出256个节点
                nn.ReLU(),
                nn.Linear(256, 256),  # 第三层，接收第一层的128个节点，输出256个节点
                nn.ReLU(),
            ),
            nn.Sequential(
                nn.Linear(28 * 28, 128),  # 第一层，将28x28的图片展平后输入，输出128个节点
                ...
                nn.Linear(256, 256),  # 第三层，接收第一层的128个节点，输出256个节点
                nn.ReLU(),
                nn.Linear(256, 256),  # 第四层，接收第一层的128个节点，输出256个节点
                nn.ReLU(),
            ),
            nn.Sequential(
                nn.Linear(28 * 28, 128),  # 第一层，将28x28的图片展平后输入，输出128个节点
                ...
                nn.Linear(256, 256),  # 第四层，接收第一层的128个节点，输出256个节点
                nn.ReLU(),
                nn.Linear(256, 256),  # 第五层，接收第一层的128个节点，输出256个节点
                nn.ReLU(),
            ),
            nn.Sequential(
                nn.Linear(28 * 28, 128),  # 第一层，将28x28的图片展平后输入，输出128个节点
        ...
        self.linear_layers = LayerChoice(layers_options,label='linear_layers')

        self.fc = nn.Linear(256, 10)  # 假设最后一个层的输出是256个特征
    def forward(self, x):
        x = self.flatten(x)
...
optimizer = torch.optim.SGD(model_space.parameters(), lr=1e-3)
...
def evaluate_model(model):
    ...
    train_dataloader = DataLoader(MNIST('data', download=True, transform=transf), batch_size=64, shuffle=True)
    test_dataloader = DataLoader(MNIST('data', download=True, train=False, transform=transf), batch_size=64)
...
exp.config.max_trial_number = 6   # spawn 3 trials at most
exp.config.trial_concurrency =6  # number of trials that can be evaluated concurrently
